    ...
    def add_competition_to_my_watchlist(self, competition_name: str, gather_all_competition_ids: Dict, defined_url:str="")->str:
        ...
        if not defined_url:
            ...
            try:
                ...
                return f"""
                        {e} ---
                        Tried to add competition to all_comps_df/

                        To add a competition, follow one of the two options. 

                        Inside add_competition_to_my_watchlist(), if you're not instatiating a competition name, 
                        then a defined_url arguement must be included (and same vice-versa).

                        ---Examples below:---
                        1. ) With competition name - defined_url is not needed because this competition exists in all_comp_info
                        ...
                        2. With a competition web address. Since this competition does NOT exist in all_comp_info, then a defined_url needs be part of the function call.
                        For these examples it doesn't matter if you include a competition name or not as it will be overwritten anyways. 
...
class Player:
    ...
    def choose_player_stats(self, competition_name: str, competition_year: str, open_close_league:str="", manual_competition_id:str="")->None:
        ...
        for row in match_rows:
            ...
            for player in player_info:
                ...
                if player_id and name and number and team_name and country and spi:
                    ...
                    # if player_id not in player_lookup_dict:
                    #     player_lookup_dict[player_id] = {
                    #         "player_name": name,
                    #         "shirt_number": number,
                    ...
                    # record = player_lookup_dict[player_id]

                    # if number != "NA" and record["shirt_number"] == "NA":
                    #     record["shirt_number"] = int(number) if number.isdigit() else -1
                    # if country != "NA" and record["country_name"] == "NA":
                    #     record["country_name"] = country
                    # if team_name != "NA" and record["team_name"] == "NA":
        ...
        print(new_df.show())
        # print(new_df.groupBy("player_id").show())
        ...
        #     all_player_team,
        #     all_player_jersey_numbers,
        #     all_player_countries
        # )

        # lookup_data = [
        #     (pid, data["player_name"], data["shirt_number"], data["country_name"], data["team_name"])
        #     for pid, data in player_lookup_dict.items()]
        ...
        # lookup_df = spark_sess.createDataFrame(lookup_data, schema=lookup_schema)

        # lookup_df.write.mode("overwrite").csv(
        #     f"{comp_dir}/{competition_name}_{competition_year}_player_lookup.csv",
        #     header=True)
    ...
    def competition_analysis6(self, competition_name: str, competition_year: str, open_close_league:str="", manual_competition_id:str="")->None:
        from pyspark.sql import SparkSession, functions as F
        from pyspark.sql.window import Window
        """
        Analyzes player statistics by computing the number of matches played and the average SPI score 
        for each player. Also fills missing player info fields (name, number, team, country) using the mode 
        (most frequent non-null value) per player_id. The results are saved in a structured CSV file.
        ...
        # Determine directory name based on inputs
        if open_close_league and not manual_competition_id:
            comp_dir = f"{competition_name}_{competition_year}_{open_close_league}_dir"
        elif not open_close_league and manual_competition_id:
            comp_dir = f"{competition_name}_{competition_year}_{manual_competition_id}_dir"
        ...
        # Columns to clean and fill missing values for
        player_info_cols = ["player_name", "player_number", "team_name", "country_name"]

        # Step 1: Clean invalid values (like 'NA', '-1', empty string) to NULL for consistency
        ...
        # Step 2: Compute matches played and average SPI per player
        matches_played_per_player = df.groupBy("player_id").count()
        avg_spi_per_player = df.groupBy("player_id").agg(F.avg("spi_score").alias("avg_spi_score"))

        # Step 3: Find mode (most frequent non-null value) for each player_info column per player_id
        def get_mode(df, col):
            return (
                ...
                .withColumn(
                    "rn",
                    F.row_number().over(
                        Window.partitionBy("player_id").orderBy(F.desc("count"))
                    ),
                )
                .filter(F.col("rn") == 1)
                .select("player_id", F.col(col).alias(f"{col}_mode"))
            )

        mode_dfs = {col: get_mode(df, col) for col in player_info_cols}

        # Step 4: Join stats with modes and original player info
        all_player_stats_df = matches_played_per_player.join(avg_spi_per_player, on="player_id", how="inner")

        player_info_unique = df.select("player_id", *player_info_cols).dropDuplicates(["player_id"])
        final_stats = all_player_stats_df.join(player_info_unique, on="player_id", how="left")

        for col in player_info_cols:
            final_stats = final_stats.join(mode_dfs[col], on="player_id", how="left")

        # Step 5: Fill NULLs in original columns with their mode values
        for col in player_info_cols:
            final_stats = final_stats.withColumn(
                col,
                F.coalesce(F.col(col), F.col(f"{col}_mode"))
            ).drop(f"{col}_mode")

        # Step 6: Select and order final columns, sort by avg_spi_score descending
        final_player_stats = final_stats.select(
            "player_id", *player_info_cols, "count", "avg_spi_score"
        ).sort(F.desc("avg_spi_score"))

        # Optional: Check null counts for debugging
        for col in player_info_cols:
            null_count = final_player_stats.filter(F.col(col).isNull()).count()
            print(f"Null count in {col} after filling: {null_count}")

        # Step 7: Save to CSV
        final_player_stats.write.mode("overwrite").csv(
            f"{comp_dir}/{competition_name}_{competition_year}_player_stats_analysis.csv",
            header=True,
        )

        print("-" * 40)
        final_player_stats.show(truncate=False)
        print("-" * 40)
    ...
    def competition_analysis5(
        self,
        competition_name: str,
        competition_year: str,
        open_close_league: str = "",
        manual_competition_id: str = "",
    ) -> None:
        from pyspark.sql import SparkSession, functions as F
        from pyspark.sql.window import Window
        import logging
        """
        Analyzes player statistics by computing the number of matches played and the average SPI score 
        for each player. Missing player info fields are filled by their most frequent non-null values.
        The results are saved in a structured CSV file.
        ...
        all_player_stats_df = matches_played_per_player.join(
            avg_spi_per_player, on="player_id", how="inner"
        )

        # Original player info, unique per player_id
        player_info = df.select(
            "player_id", "player_name", "player_number", "team_name", "country_name"
        ).dropDuplicates(["player_id"])

        # Helper function to get mode (most frequent non-null, valid value) per player_id for a column
        def get_mode(df, col):
            return (
                df.filter(
                    (F.col(col).isNotNull())
        ...
        # Compute modes for player info columns
        mode_player_name = get_mode(df, "player_name")
        mode_player_number = get_mode(df, "player_number")
        mode_team_name = get_mode(df, "team_name")
        mode_country_name = get_mode(df, "country_name")

        # Join all stats + original info + modes
        final_stats = (
            all_player_stats_df.join(player_info, on="player_id", how="left")
            .join(mode_player_name, "player_id", "left")
            .join(mode_player_number, "player_id", "left")
            .join(mode_team_name, "player_id", "left")
            .join(mode_country_name, "player_id", "left")
        )

        # Fill null or invalid player info columns with their mode
        for col in ["player_name", "player_number", "team_name", "country_name"]:
            final_stats = final_stats.withColumn(
                col,
                F.when(
                    (F.col(col).isNull())
        ...
        logging.info("All players' stats have been saved successfully!")
        spark.stop()



    def competition_analysis4(self, competition_name: str, competition_year: str, open_close_league: str = "", manual_competition_id: str = "") -> None:
        """
        Analyzes player statistics by computing the number of matches played and the average SPI score 
        ...
        # Compute counts and average spi_score
        matches_played_per_player = df.groupBy("player_id").count()
        avg_spi_per_player = df.groupBy("player_id").agg(F.avg("spi_score").alias("avg_spi_score"))

        all_player_stats_df = matches_played_per_player.join(avg_spi_per_player, on="player_id", how="inner")

        # Function to get mode (most frequent non-null, non-NA, non -1) per player_id for a column
        def get_mode(df, col):
            return (
                df.filter(
                    (F.col(col).isNotNull()) & (F.col(col) != "NA") & (F.col(col) != "-1")
                )
                .groupBy("player_id", col)
                .count()
                .withColumn(
        ...
        # Get modes for each player info column
        mode_player_name = get_mode(df, "player_name")
        mode_player_number = get_mode(df, "player_number")
        mode_team_name = get_mode(df, "team_name")
        mode_country_name = get_mode(df, "country_name")

        # Join modes back to aggregated stats
        final_stats = (
            all_player_stats_df.join(mode_player_name, "player_id", "left")
            .join(mode_player_number, "player_id", "left")
            .join(mode_team_name, "player_id", "left")
            .join(mode_country_name, "player_id", "left")
        )

        # Fill missing or invalid values with mode values
        for col in ["player_name", "player_number", "team_name", "country_name"]:
            final_stats = final_stats.withColumn(
                col,
                F.when(
                    (F.col(col).isNull()) | (F.col(col) == "NA") | (F.col(col) == "-1"),
                    F.col(f"{col}_mode"),
                ).otherwise(F.col(col)),
            ).drop(f"{col}_mode")
    ...
    def competition_analysis3(self, competition_name: str, competition_year: str, open_close_league:str="", manual_competition_id:str="")->None:
        """
        Analyzes player statistics by computing the number of matches played and the average SPI score 
        for each player. The results are saved in a structured CSV file.
        ...
        def count_matches_played():
            return df.groupBy(["player_id"]).count()

        def compute_average_spi():
            return df.groupBy(["player_id"]).agg(F.avg("spi_score").alias("avg_spi_score"))

        matches_played_per_player = count_matches_played()
        avg_spi_per_player = compute_average_spi()

        all_player_stats_df = matches_played_per_player.join(avg_spi_per_player, how="inner", on=["player_id"]).sort(F.desc("avg_spi_score"))

        df_2_subset = df.select("player_id", "player_name", "player_number", "team_name", "country_name")

        final_player_stats = all_player_stats_df.join(df_2_subset, on="player_id", how="inner")

        # Columns to fill
        columns_to_fill = ["player_number", "team_name", "country_name"]

        def get_mode_per_player(df, column):
            return (
                df.filter((F.col(column).isNotNull()) & (F.col(column) != "NA") & (F.col(column) != "-1"))
                ...
                .filter(F.col("rank") == 1)
                .select("player_id", F.col(column).alias(f"{column}_mode"))
            )

        df_filled = final_player_stats

        for col in columns_to_fill:
            mode_df = get_mode_per_player(final_player_stats, col)
            df_filled = df_filled.join(mode_df, on="player_id", how="left").withColumn(
                col,
                F.when(
                    (F.col(col).isNull()) | (F.col(col) == "NA") | (F.col(col) == -1),
                    F.col(f"{col}_mode")
        ...
        df_filled = df_filled.dropDuplicates(["player_id"])

        df_filled.write.mode("overwrite").csv(f"{comp_dir}/{competition_name}_{competition_year}_player_stats_analysis.csv", header=True)
    ...
    def competition_analysis2(self, competition_name: str, competition_year: str, open_close_league:str="", manual_competition_id:str="")->None:
        """
        Analyzes player statistics by computing the number of matches played and the average SPI score 
        for each player. The results are saved in a structured CSV file.
        ...
        df_2_subset = df.select("player_id", "player_name", "player_number", "team_name", "country_name").dropDuplicates(["player_id"])
